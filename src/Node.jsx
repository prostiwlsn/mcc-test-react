      ...
      <div className="mt-1 flex flex-col">
        <div className="flex flex-row align-middle group">
          ...
          <div className="text-xs flex flex-row align-middle parent *:px-2 *:border-2 *:rounded-3xl *:transition-all *:ml-2">
            {(isEditingToggled && id != editedNode) && 
              <button className="opacity-30 hover:!opacity-100 group-hover:opacity-50 hover:text-amber-300 hover:border-amber-300" 
              onClick={() => setValueEdit(id)}>
                Edit
              </button>}
            {(isEditingToggled && id == editedNode) && <>
              ...
              onClick={() => { setValueEdit(id); setEditNodeValue(!editNodeValue);}}>
                {editNodeValue ? "Save value" : "Edit value"}
              </button>
